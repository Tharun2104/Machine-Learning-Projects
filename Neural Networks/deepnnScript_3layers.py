...
def create_multilayer_perceptron():
    ...
    n_hidden_3 = 128  # 3rd layer number of features (new layer)
    ...
    weights = {
        ...
        'h3': tf.Variable(tf.random_normal([n_hidden_2, n_hidden_3])),  # weights for new layer
        'out': tf.Variable(tf.random_normal([n_hidden_3, n_classes]))
    }
    biases = {
        'b1': tf.Variable(tf.random_normal([n_hidden_1])),
        'b2': tf.Variable(tf.random_normal([n_hidden_2])),
        'b3': tf.Variable(tf.random_normal([n_hidden_3])),  # bias for new layer
    ...
    # Third hidden layer with RELU activation (new layer)
